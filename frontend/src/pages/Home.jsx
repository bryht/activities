...
export default function Home() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="bg-gradient-to-b from-brand-100 to-rose-50/40 px-4 pb-8 pt-8 text-center sm:px-6 md:pb-12 md:pt-14">
        <KidGoMark className="mx-auto mb-4 h-20 w-20 drop-shadow-sm md:h-24 md:w-24" />
        <h1 className="text-2xl font-extrabold leading-tight text-slate-900 sm:text-3xl md:text-4xl">
          Find playmates for your little one
        </h1>
        <p className="mx-auto mt-2 max-w-xs text-sm text-slate-600 sm:max-w-md sm:text-base md:mt-4 md:max-w-xl md:text-lg">
        ...
        <div className="mx-auto mt-5 flex max-w-xs flex-col gap-2 sm:mt-7 sm:max-w-md sm:flex-row sm:justify-center">
          ...
          <Link
            to="/activities"
            className="inline-block w-full rounded-full bg-white px-5 py-3 font-semibold text-brand-600 shadow-sm ring-1 ring-brand-100 transition hover:shadow-md sm:flex-1"
          >
            Browse activities
          </Link>
        </div>
        ...
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 no-scrollbar sm:mx-0 sm:grid sm:grid-cols-3 sm:overflow-visible sm:px-0 lg:grid-cols-6">
          {groups.map((g) => (
            <Link
              ...
              className="flex min-w-[88px] flex-col items-center gap-1 rounded-2xl bg-white p-3 text-center shadow-sm ring-1 ring-slate-100 transition hover:shadow-md sm:min-w-0"
            >
